package com.jacobarchambault.courselist;
...
public class CourseList extends JFrame {
	...
	public void createLabels() {
		...
		courseNoLabel = new JLabel(
				"Course Number: ",
				SwingConstants.RIGHT);
		sectionLabel = new JLabel(
				"Section Number: ",
				SwingConstants.RIGHT);
		enrolledLabel = new JLabel(
				"Number Enrolled: ",
				SwingConstants.RIGHT);
	}
	...
	private void addCourse() {
		...
		try {
			...
			if (prefix.length() == 0 || strCourseNo.length() == 0 || section.length() == 0 || strEnrolled.length() == 0)
			...
			JOptionPane.showMessageDialog(
					this,
					"courseNo and enrolled must be numeric");
		}
	}
	...
	private void loadCourse() {
		...
		// The CourseData getAll() method is called and returns a value assigned to the
		// courses ArrayList
		courses = CourseData.getAll();
		for (int i = 0; i < courses.size(); i++) {
			aCourse = (Course) courses.get(
					i);
			courseList.addItem(
					aCourse.getPrefix() + " " + aCourse.getCourseNo() + " " + aCourse.getSection() + " has "
							+ aCourse.getEnrolled() + " students enrolled");
		}
	}
